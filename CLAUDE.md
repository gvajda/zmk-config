...
`build.yaml` at repo root defines the GitHub Actions matrix: left half (with ZMK Studio via the `studio-rpc-usb-uart` snippet — Studio only runs on the central half), right half, and `settings_reset` images for both halves.
...
| `build.yaml` | CI matrix (left/right halves + settings_reset) |
...
- `hsl` / `hsr` — shift-only variants with a shorter `require-prior-idle-ms` (100 vs 150)
